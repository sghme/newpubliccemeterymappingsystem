...
class User1 {
    ...
    // Assuming your table name is 'tbluser'
    public static $tblname = 'tbluser';

    // ... (your existing code)

    public static function userAuthentication($username, $password) {
        global $mydb;
        $mydb->setQuery("SELECT * FROM `tbluser` WHERE `username` = '{$username}' AND `password` = '{$password}'");
        $cur = $mydb->executeQuery();
        ...
        if ($row_count == 1) {
            ...
            // Check the user's status to allow or disallow login
            if ($user_found->status == 'approved') {
	...
	public function updateStatus($id, $status) {
		global $mydb;
		$sql = "UPDATE " . self::$tblname . " SET status = '{$status}' WHERE id = '{$id}'";
		$mydb->setQuery($sql);
	
		if (!$mydb->executeQuery()) {
			echo "Error: " . $mydb->getLastError();
			return false;
		}
